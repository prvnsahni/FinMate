...
An **ADR** is a short note that records _why_ an important decision was made — so future engineers don't accidentally undo it. FinMate wrote ADRs only for the big, hard-to-reverse choices: how data is locked, who holds the keys, how the AI is kept away from your private data, how logging-in works on web vs phone, how deletion really works, and — most importantly — how to add all of this **without breaking the money app people already use**. Each ADR is written twice: a simple explanation anyone can read, then the technical detail. The single most important idea across all of them: _secure and improve the existing product without unnecessarily breaking it._
...
_ADRs record why; the SRS records what; the architecture documents record how; the current baseline records what must not be broken._
