...
_Simple:_ one guarded door with many checks; if any check fails, nothing is sent. _Technical:_ a mandatory ordered gate; failure at any stage → fail-closed (§21).
...
_Simple:_ the brain gets labelled hints, never the databases. _Technical:_ signals travel via the durable outbox with consent tags; INTELLIGENCE stores derived facts under its own server-managed key, never raw source data.
...
When you type a question to the assistant, FinMate treats your words as _just a question_ — never as secret commands. The assistant only sees a small number summary plus your question, and it can't reach into anything else.
...
_Simple:_ one guarded door between your data and any AI. _Technical:_ intent-driven, projection-only egress through a single audited firewall.
...
_End of Document #5 (FROZEN). STOP — not proceeding to the Threat Model or the SRS._
