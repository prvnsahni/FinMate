...
**Purpose:** the **compatibility baseline** for the future SRS. _"Current system reality must be known before target system requirements are written."_ Describes what **actually exists today**, verified from the repository — not what the frozen architecture documents _plan_.
...
**Glossary:** _E2EE_ = end-to-end encrypted (only the user can read). _Zone 2_ = server-readable, protected. _ZK_ = zero-knowledge (server can't read). _TIK_ = temporary invite key. _PWK_ = public wrapping key.
...
> **⟳ SEC-KI1 / KI-1 STATUS CORRECTION — 2026-08-13 (additive; the historical baseline statements above — incl. "no remediation in repo", §11 "Known gap", §18, §20, §25 — are preserved verbatim as the point-in-time finding).** A later, deeper repository verification ([FINMATE_SEC_KI1_VERIFICATION.md](FINMATE_SEC_KI1_VERIFICATION.md)) supersedes the KI-1 line specifically: **remediation IS present in the repo** — the canonical group-key `versionId` path is honored end-to-end (`getMyGroupKey` serves the requested version, SUPERSEDED served, REVOKED rejected, caller-scoped wrapped keys; write path stamps the client-declared version), **fixed 2026-07-17** on branch `Expense-module0a` (`gap-tracker.md` ENC-002/EXP-002/EXP-003 = Done), with unit tests (`groups.service.spec.ts:1289/1210/1335/1344`). **Historical canonical expenses remain decryptable after normal rotation** → KI-1 is **MITIGATED/VERIFIED** for the canonical path (was recorded OPEN in this baseline). **M-KEYVER = VERIFY-ONLY — no migration/re-encryption/production change.** **Residual (display-only, not canonical data loss):** GRP-007 — group history-log ciphertext titles in `audit_logs.metadataJson` lack a version stamp → placeholder after rotation. **Separate open items:** GRP-005 (leaver retains cached wrapped key), legacy **NULL-`versionId`** rows (REQUIRES PRODUCTION VERIFICATION), **REVOKED semantics** — [PRODUCT/SECURITY DECISION REQUIRED]. All **other** SEC gaps in the list above remain OPEN and unchanged. 10. **ENG-UNKNOWN:** notes functional status, SW cache groups, prod CORS, deployed refresh storage, IDOR coverage, version/recurring-split columns, recurring/attachment prod rows, investment-AI policy, AI-memory retention. 11. **Conflicts with frozen docs:** 6 recorded (all CURRENT-vs-TARGET gaps; no frozen-doc change). 12. **New product questions:** none new (OQ-1 bank aggregation, OQ-2 proactivity carried from Doc #10). 13. **Could break existing functionality:** only the refresh-transport change (AU-1) has a **potential** breaking edge — mitigated by dual-emit + min-version (AU-4); everything else is additive.
...
_End of Document #11 (CURRENT baseline). STOP — the SRS is next, and must build on this verified reality._
