...
- **Decision:** Key rotation is event-driven (member removal, suspected compromise, key-loss/recovery, defined security events); no calendar rotation unless a later threat model justifies it. V1 does **not** claim retroactive revocation — a removed member may retain access to data they already decrypted/cached; rotation protects future data. Fix the existing group-key `versionId`-ignored bug (KI-1) before more encrypted domains rely on rotation. *(⟳ 2026-08-13: the KI-1 prerequisite is **VERIFIED satisfied** in the repository — see the SEC-KI1 status correction two entries below and [FINMATE_SEC_KI1_VERIFICATION.md](FINMATE_SEC_KI1_VERIFICATION.md).)*
...
*End of Frozen Decision Ledger. Change control: any modification to a LOCKED item requires a new dated entry here plus an ADR. This ledger governs the documents that follow it in the stack.*
