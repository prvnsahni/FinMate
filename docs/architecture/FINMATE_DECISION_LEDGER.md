...
- **Decision:** Key rotation is event-driven (member removal, suspected compromise, key-loss/recovery, defined security events); no calendar rotation unless a later threat model justifies it. V1 does **not** claim retroactive revocation — a removed member may retain access to data they already decrypted/cached; rotation protects future data. Fix the existing group-key `versionId`-ignored bug (KI-1) before more encrypted domains rely on rotation.
