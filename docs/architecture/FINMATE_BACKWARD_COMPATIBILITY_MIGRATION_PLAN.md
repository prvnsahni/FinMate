...
**Critical principle:** *"Secure and improve the existing product without unnecessarily breaking the product."* FinMate is a **live production application**; existing production data **must be assumed to exist**. **Never** a destructive clean-slate migration for architectural cleanliness alone (GOV-1/GOV-2, ADR-001).
...
1. **P0 security prerequisites (do first):** SEC-W1 (git-history blobs + secret scanning), SEC-W2 (stop logging tokens/email/IP), SEC-W3 groundwork. *These gate anything that ships new auth/log surfaces.*
...
**MIG-01 — Overall migration strategy.** *Simple:* add first, run old+new together, convert on-device, verify, retire last. *Technical:* additive-first, mixed-state, client-backfill, flag-gated, security-sequenced.
```mermaid
flowchart LR
...
**MIG-02 — E2EE mixed-state.** See §6 diagram. *Simple:* labels tell old from locked. *Technical:* marker branch + client backfill; atomic marker+ciphertext.
...
**MIG-06 — Group-key verify.** *Simple:* check the version handling; don't re-encrypt history. *Technical:* confirm endpoint honors `versionId` + define REVOKED-version policy.
```mermaid
flowchart TD
...
**MIG-07 — Attachment migration.** *Simple:* stop saving new plaintext names. *Technical:* additive stop-populate; keep reading existing.
```mermaid
flowchart LR
...
**MIG-09 — Rollout pipeline.** *Simple:* dev→test→staging→canary→prod, checks each step. *Technical:* entry/verify/rollback/exit per stage (§21).
```mermaid
flowchart LR
...
*Acting as a careless/hostile developer. Each successful attack strengthens the document; none changes code.*
...
*End of Document #17 (Backward Compatibility & Migration Plan). STOP — no migrations created or executed, no schema/production change, no implementation tickets.*
