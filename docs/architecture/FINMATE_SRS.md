...
**Governing principle:** _"Secure the existing product without unnecessarily breaking the product."_ Existing production functionality is a **protected baseline**.
...
Critical requirements use measurable criteria. **Example (FIN-002):** _"Given the same payer, participants, split configuration and refund state as current production, the target implementation MUST produce the same balance result."_ Each requirement row above carries a testable acceptance column.
...
_(Every requirement ID above traces to a source document + decision/threat ID; this table shows the pattern for the highest-priority items.)_
...
_End of FinMate SRS v1.0 (Document #13) — FROZEN 2026-08-12. No ADRs, API contracts, migration scripts, or implementation created. Decision Ledger and Documents #2–#12 unmodified._
