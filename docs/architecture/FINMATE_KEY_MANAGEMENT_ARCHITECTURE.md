...
**Central question this document answers:** _"Who holds the key to unlock each type of FinMate data?"_
...
> **⟳ SEC-KI1 STATUS CORRECTION — 2026-08-13 (additive; the prerequisite note above is preserved as historical).** Repository verification ([FINMATE_SEC_KI1_VERIFICATION.md](FINMATE_SEC_KI1_VERIFICATION.md)) established the prerequisite is **satisfied**: `getMyGroupKey` honors `versionId` (fixed 2026-07-17), old ciphertext keeps its version stamp and **decrypts with the SUPERSEDED key** (exactly as this section's rotation diagram already specifies), REVOKED is rejected, and caller-specific wrapped keys are returned. **Historical canonical expenses remain decryptable after normal rotation** → **MITIGATED/VERIFIED** (was P2 OPEN). **M-KEYVER = VERIFY-ONLY — no migration, no re-encryption, no key rotation, no production change.** Residual (display-only): **GRP-007** (history-log ciphertext title without a version stamp). Separate open items: GRP-005 (leaver-key), legacy NULL-`versionId` (needs prod verification), **REVOKED semantics** — [PRODUCT/SECURITY DECISION REQUIRED]; note `REVOKED` is currently **never set by any code path** (rotation uses `SUPERSEDED`).
...
_Simple:_ your device does all the locking and unlocking; the server just holds the locked box. _Technical:_ AES-256-GCM data key wrapped under the master; server persists ciphertext + wrapped blob with no decryption path.
...
_End of Document #4 (FROZEN). STOP — not proceeding to the Threat Model or the SRS._
