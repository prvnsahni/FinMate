import { computeLevel, isVisible, rankCandidates } from './notification-ranking';
...
describe('notification-ranking (NOT-001/003/006)', () => {
  describe('computeLevel', () => {
    it('maps importance to a base L1–L5', () => {
      expect(computeLevel(candidate({ importance: 'critical', urgency: 0 }))).toBe(1);
      expect(computeLevel(candidate({ importance: 'high', urgency: 0 }))).toBe(2);
      expect(computeLevel(candidate({ importance: 'useful', urgency: 0 }))).toBe(3);
      expect(computeLevel(candidate({ importance: 'low', urgency: 0 }))).toBe(4);
      expect(computeLevel(candidate({ importance: 'optional', urgency: 0 }))).toBe(5);
    });

    it('high urgency + confidence promotes one level', () => {
      expect(
        computeLevel(candidate({ importance: 'high', urgency: 0.9, confidence: 0.9 })),
      ).toBe(1);
    });

    it('low confidence demotes one level (clamped to L5)', () => {
      expect(
        computeLevel(candidate({ importance: 'low', urgency: 0, confidence: 0.3 })),
      ).toBe(5);
      expect(
        computeLevel(candidate({ importance: 'optional', urgency: 0, confidence: 0.3 })),
  ...
  describe('rankCandidates', () => {
    it('orders by level then deterministic tie-breakers, and minimizes shape', () => {
      const ranked = rankCandidates(
        [
          candidate({ id: 'b', importance: 'high', urgency: 0.5, confidence: 0.9 }),
          candidate({ id: 'a', importance: 'high', urgency: 0.5, confidence: 0.9 }),
      ...
      expect(Object.keys(ranked[0]).sort()).toEqual(
        ['actionable', 'category', 'id', 'level', 'message', 'observedAt', 'title'].sort(),
      );
    });
    ...
    it('urgency then confidence break ties within a level', () => {
      const ranked = rankCandidates(
        [
          candidate({ id: 'lo', importance: 'high', urgency: 0.5, confidence: 0.7 }),
          candidate({ id: 'hi', importance: 'high', urgency: 0.7, confidence: 0.7 }),
        ],
        'standard',
      );
