...
describe('DOC-2 receipt fixtures — reconciliation via computeReconciliation', () => {
  it('grocery-balanced: items sum to the printed total → BALANCED (685 = 685)', () => {
    ...
    const r = computeReconciliation(
      f.printedTotal,
      f.expected.lineItems.map((li) => li.lineTotal),
    );
...
describe('DOC-2 fixtures — hygiene & coverage', () => {
  it('cover image, text-PDF and scanned-PDF source types', () => {
    ...
    const textPdf = RECEIPT_FIXTURES.filter(
      (f) => f.sourceType === 'pdf' && f.pdfHasTextLayer === true,
    );
    const scannedPdf = RECEIPT_FIXTURES.filter(
      (f) => f.sourceType === 'pdf' && f.pdfHasTextLayer === false,
    );
  ...
  it('contain no secrets/PII patterns (synthetic only)', () => {
    ...
    expect(blob).not.toMatch(
      /password|token|secret|api[_-]?key|@[a-z0-9.-]+\.[a-z]{2,}/i,
    );
  });
