...
describe('selectAdapterKind (image vs text-PDF vs scanned-PDF)', () => {
  it('routes images to the image adapter', () => {
    expect(
      selectAdapterKind({ mimeType: 'image/jpeg', sourceType: 'image' }),
    ).toBe('image');
  });
  ...
  it('defaults a PDF (no probe signal) to pdf_text (preferred path)', () => {
    expect(
      selectAdapterKind({ mimeType: 'application/pdf', sourceType: 'pdf' }),
    ).toBe('pdf_text');
  });

  it('returns none for unsupported input', () => {
    expect(
      selectAdapterKind({ mimeType: 'text/plain', sourceType: 'unknown' }),
    ).toBe('none');
  });
});
