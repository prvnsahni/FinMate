...
describe('PublicSharesService (PUBLIC-1B)', () => {
  ...
  const asRole = (role: string) =>
    memberRepo.findOne.mockResolvedValue({ id: 'm', role, joinStatus: 'active' } as unknown as GroupMember);
  ...
  it('1/2. owner and admin can create a share', async () => {
    asRole('owner');
    await expect(service.create(USER, GROUP, {})).resolves.toMatchObject({ status: 'active' });
    shares = [];
    asRole('admin');
    await expect(service.create(USER, GROUP, {})).resolves.toMatchObject({ status: 'active' });
  });
  ...
  it('3/4. a plain member and a viewer cannot create (Forbidden)', async () => {
    asRole('member');
    await expect(service.create(USER, GROUP, {})).rejects.toBeInstanceOf(ForbiddenException);
    asRole('viewer');
    await expect(service.create(USER, GROUP, {})).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it('6/7/8. create returns the raw token ONCE; DB stores only its sha256 hash; no hash in the response', async () => {
    ...
    expect(saved.tokenHash).toBe(createHash('sha256').update(res.token).digest('hex'));
  ...
  it('rejects a second create while a share is already active (one active share)', async () => {
    asRole('owner');
    await service.create(USER, GROUP, {});
    await expect(service.create(USER, GROUP, {})).rejects.toBeInstanceOf(ConflictException);
  ...
  it('14. revoke invalidates the active share immediately', async () => {
    asRole('owner');
    await service.create(USER, GROUP, {});
    await expect(service.revoke(USER, GROUP)).resolves.toEqual({ revoked: true });
    expect(activeShares().length).toBe(0);
  });

  it('15. revoke is idempotent when nothing is active (safe no-op)', async () => {
    asRole('owner');
    await expect(service.revoke(USER, GROUP)).resolves.toEqual({ revoked: false });
  });
