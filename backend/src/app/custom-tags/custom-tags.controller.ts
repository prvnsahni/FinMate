...
export class CustomTagsController {
  ...
  /** List the caller's own active personal custom tags. */
  @Get()
  async listPersonal(@Req() req: AuthedRequest) {
    const tags = await this.customTagsService.listPersonal(req.user.id);
...
export class GroupCustomTagsController {
  ...
  /** List a group's active custom tags (member-only). */
  @Get()
  async listGroup(
    @Param('groupId', new ParseUUIDPipe({ version: '4' })) groupId: string,
    @Req() req: AuthedRequest,
  ) {
    const tags = await this.customTagsService.listGroup(req.user.id, groupId);
    return new SuccessResponse('Custom tags retrieved successfully', tags);
  }
}
