...
describe('AddCustomTags1720200000000', () => {
  ...
  it('stores the name E2EE-only — no plaintext/normalized/hash/search companion', async () => {
    ...
    expect(sql).not.toMatch(/normalized_key|name_hash|name_search|plaintext_name|"name"\s+(VAR|TEXT)/i);
  });

  it('enforces the scope invariant with a CHECK constraint', async () => {
    const sql = await runMigration('up');
    expect(sql).toMatch(/CONSTRAINT "chk_custom_tags_scope" CHECK/);
    expect(sql).toMatch(/"scope_type" = 'personal' AND "owner_user_id" IS NOT NULL AND "group_id" IS NULL/);
    expect(sql).toMatch(/"scope_type" = 'group' AND "group_id" IS NOT NULL AND "owner_user_id" IS NULL/);
  });

  it('uses safe FK delete behaviour', async () => {
    const sql = await runMigration('up');
    // Owner/group deletion removes the DEFINITION only.
    expect(sql).toMatch(/"owner_user_id"[\s\S]*REFERENCES "users"\("id"\) ON DELETE CASCADE/);
    expect(sql).toMatch(/"group_id"[\s\S]*REFERENCES "groups"\("id"\) ON DELETE CASCADE/);
    // Creator + key-version references null out, never cascade.
    expect(sql).toMatch(/"created_by_user_id"[\s\S]*REFERENCES "users"\("id"\) ON DELETE SET NULL/);
    expect(sql).toMatch(/"group_key_version_id"[\s\S]*REFERENCES "group_key_versions"\("id"\) ON DELETE SET NULL/);
  });
  ...
  it('performs NO backfill and touches NO financial column', async () => {
    ...
    expect(sql).not.toMatch(/amount_total|currency|paid_by|split|refund|settlement|balance/i);
  });
  ...
  it('indexes the future access patterns', async () => {
    ...
    expect(sql).toMatch(/CREATE INDEX[\s\S]*"custom_tags" \("scope_type", "status"\)/);
  });

  it('down() is reversible — drops only the added column and new table', async () => {
    const sql = await runMigration('down');
    expect(sql).toMatch(/ALTER TABLE "expense_tags" DROP COLUMN IF EXISTS "tag_scope"/);
